...
def get_dict(type: object) -> object:
    global DICT_INIT
            ...
            if installed:
                ...
                try:
                    ...
                    with open(tmp_file+tmp_suffix,'wb') as temp_cache_file:
                        pickle.dump(DICTS.DEFAULT_DICT, temp_cache_file, protocol = 0)
                    if os.name=='nt':
                        import shutil
...
class SurnameCutting(CuttingBase):
    ...
    def cut_stage3(self, cuttor, sentence, graph, start, end):
        ...
        if klen >= 2 and self.dict.get(sentence[start:start+1]):
            # TODO for this prob
            graph.add_edge(start,start+2, cuttor.refer_prob*0.8)
        if klen > 2 and self.dict.get(sentence[start:start+1]):
            graph.add_edge(start,start+3, cuttor.refer_prob*1.2)
        if klen > 2 and self.dict.get(sentence[start:start+2]):
            graph.add_edge(start,start+3, cuttor.refer_prob*1.2)
        if klen > 3 and self.dict.get(sentence[start:start+2]):
...
class SuffixCutting(CuttingBase):
    ...
    def cut_stage4(self, cuttor, sentence, graph, contex):
        ...
        klen = path[i]-path[i-1]
        if klen >= 1 and klen <= 2 and n2 > 2 and self.dict.get(sentence[path[i-1]:path[i]]):
            j = n2-1
            while j > 1:
                klen2 = new_path[j]-new_path[j-1]
                if klen2 == 1 and not self.stop_dict.get(sentence[new_path[j-1]:new_path[j]]):
                    new_path.pop()
                    contex['single'] -= 1
                    modify = True
                    j -= 1
