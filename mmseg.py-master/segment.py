...
def _chunk_avg_len(chunk):
    return sum(map(len, chunk)) / float(len(chunk))


def _chunk_var_len(chunk):
    avg_len = _chunk_avg_len(chunk)
    squares = map(lambda c: (len(c) - avg_len) ** 2, chunk)
    return sum(squares) / float(len(chunk))


def _chunk_tf_sum(lex):
    def calculate_sum(chunk):
        chunk = filter(lambda w: len(w) == 1, chunk)
        tf_list = map(lex.term_frequency, chunk)
        return sum(tf_list)
...
def _create_rule(func, get_max=True):
    def rule(chunks):
        ...
        target = max_or_min(map(func, chunks))
